...
#+--- Bars LEFT ---+
# Session name
tmux set -g status-left "#[fg=${THEME[surface0]},bg=${THEME[sapphire]},bold] #{?client_prefix,󰵚 ,#[dim]󰤂 }#[bold,nodim]aya "

#+--- Windows ---+
# Focus
tmux set -g window-status-current-format "$RESET#[fg=${THEME[blue]},bg=${THEME[surface1]},dim] #{?#{==:#{pane_current_command},ssh},#[fg=${THEME[green]}]󰣀#[fg=${THEME[blue]}] , }#[fg=${THEME[overlay]},bold,nodim]$window_number#W#[nobold]#{?window_zoomed_flag, $zoom_number, $custom_pane}#[fg=${THEME[green]},bold]#{?window_last_flag, , }"
